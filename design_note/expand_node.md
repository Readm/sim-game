...
### 第一阶段：基础数据结构[完成]
1. 在`Node`类中添加展开状态标志
    + Node的状态中，在p_state中添加一个d_state（类型为DisplayState）。也会被序列化和反序列化。
    + DispalyState中应该有是否被Expanded属性代表是否被展开。
    + 并且也有坐标信息，默认为-1，代表未指定位置，由GUI决定。
4. 添加视图层级标记，在初始化时，向父节点逐个查询，初始化为到顶层节点（NetWork）的距离。
